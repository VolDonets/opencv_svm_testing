...
void load_images(const std::string &images_path, const std::string &image_resolution, const std::string boxes_path,
...
int main() {
    ...
    std::vector<cv::Mat> ok_train_rois, no_ok_train_rois, no_ok_train_rois_full;
    ...
    load_images(no_ok_images_path, image_resolution, no_ok_boxes_path, no_ok_train_rois_full);
    ...
    if (train_twice) {
        ...
        for (size_t i = 0; i < no_ok_train_rois.size(); i++) {
            if (no_ok_train_rois[i].cols >= box_width && no_ok_train_rois[i].rows >= box_height)
                my_hog.detectMultiScale(no_ok_train_rois[i], detections, foundWeights);
            else
                detections.clear();
            for (size_t j = 0; j < detections.size(); j++) {
                cv::Mat detection = no_ok_train_rois[i](detections[j]).clone();
                resize(detection, detection, cv::Size(box_width, box_height), 0, 0, cv::INTER_LINEAR_EXACT);
                no_ok_train_rois.push_back(detection);
            }
        }
        ...
        computeHOGs( cv::Size(box_width, box_height), ok_train_rois, gradient_lst, flip_samples );
        int positive_count = gradient_lst.size();
        std::cout << "...[done] ( positive count : " << positive_count << " )" << std::endl;
        ...
        std::cout << "...[done] ( negative count : " << negative_count << " )" << std::endl;
        labels.clear();
        ...
        for (size_t i = 0; i < img_lst.size(); i++) {
            if (img_lst[i].cols >= wsize.width && img_lst[i].rows >= wsize.height) {
                cv::Rect r = cv::Rect((img_lst[i].cols - wsize.width) / 2,
                                      ...
                                      wsize.width,
                                      wsize.height);
                cv::cvtColor(img_lst[i](r), gray, cv::COLOR_RGB2GRAY);
